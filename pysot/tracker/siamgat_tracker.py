...
class SiamGATTracker(SiameseTracker):
    ...
    def init(self, img, bbox):
        ...
        #ROI是Region of Interest的简写，指的是在“特征图上的框”；这里也就是
...
#粗定位
    ...
    def getCenter(self, coarseSco, fineSco, scale_score, lrtbs):
        ...
        # fine location
        CRowUp, CColUp = np.unravel_index(fineSco.argmax(), fineSco.shape) #默认order为C,取得fineSco中最大值的坐标
    ...
    def track(self, img):
        ...
        args:
            img(np.ndarray): BGR image
        return:
            bbox(list):[x, y, width, height] 左上角的点xy坐标，宽高
        """
        w_z = self.size[0] + cfg.TRACK.CONTEXT_AMOUNT * np.sum(self.size)
        h_z = self.size[1] + cfg.TRACK.CONTEXT_AMOUNT * np.sum(self.size)
        s_z = np.sqrt(w_z * h_z)
        ...
        #  cfg.TRACK.INSTANCE_SIZE为什么会读出来255，悬而未决，在这里直接将其改为287数值
        # 裁剪出来的搜索区域[1,3,287,287]
        x_crop = self.get_subwindow(img, self.center_pos,
                                    287,
                                    round(s_x), self.channel_average)


        #这里的outputs包括了cls,cen,lrtbs
        outputs = self.model.track(x_crop)

        # self._convert_cls(outputs['cls'])是[1,25,25],squeeze将第0个维度删除变成[25,25]
        ...
        # 4x4像素邻域的双3次插值变成upsize 大小 [193,193]
