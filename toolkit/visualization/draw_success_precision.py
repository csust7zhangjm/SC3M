...
def draw_success_precision(success_ret, name, videos, attr, precision_ret=None,
    ...
    plt.savefig('./img/OTB100{}.pdf'.format(attr))
    plt.show()
    ...
    if precision_ret:
        ...
        ax.autoscale(enable=True, axis='linestyleboth', tight=True)
        xmin, xmax, ymin, ymax = plt.axis()
        ax.autoscale(enable=False)
        ymax += 0.03
        ymin = 0
        ...
        plt.yticks(np.arange(ymin, ymax, 0.1))
        ax.set_aspect((xmax - xmin)/(ymax-ymin))
        plt.savefig('./img/OTB100_{}.pdf'.format(attr))
        plt.show()
    ...
    if norm_precision_ret:
        ...
        plt.yticks(np.arange(ymin, ymax, 0.1))
        ax.set_aspect((xmax - xmin)/(ymax-ymin))
        plt.savefig('./img/OTB100_{}.png'.format(attr))
        plt.show()
